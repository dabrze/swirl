...
install_from_swirl <- function(course_name){
  ...
  if(!is.character(course_name)) {
    stop("Argument 'course_name' musi byæ otoczony cudzys³owami (musi byæ stringiem)!")
  }
  ...
  if(length(unzip_list) == 0) {
    stop(paste0("Nie znaleziono modu³u '", course_name, "' w repozytorium! ",
                "Upewnij siê, ¿e poda³eœ poprawn¹ nazwê repozytorium i spróbuj jeszcze raz."))
  }
  ...
  if(file.copy(dirs_to_copy, get_swirl_option("courses_dir"), recursive=TRUE)){
    ...
    swirl_out("Nie uda³o siê zainstalowaæ modu³u.", skip_after=TRUE)
  }
...
install_course_zip <- function(path, multi=FALSE, which_course=NULL){
  if(!is.logical(multi) || is.na(multi)) {
    stop("Argument 'multi' musi mieæ wartoœæ TRUE lub FALSE.")
  }
  if(!multi && !is.null(which_course)) {
    stop("Argument 'which_course' powinien byæ podany tylko gdy 'multi' ma wartoœæ TRUE.")
  }
  if(multi){
    # Find list of files not in top level directory
    file_names <- unzip(path, list=TRUE)$Name
    ...
    if(!is.null(which_course)) {
      ...
      nomatch <- match_ind < 0
      if(any(nomatch)) {
        stop("Nie znaleziono modu³u ", sQuote(which_course[nomatch][1]), " we wskazanym katalogu. Uwaga, nazwy modu³ów s¹ \"case sensitive\"!")
      }
...
install_course_directory <- function(path){
  ...
  if(length(garbage_result) > 1000){
    stop("Za du¿o plików w katalogu modu³u!")
  }
