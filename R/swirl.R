...
bye <- function(){
  removeTaskCallback("mini")
  swirl_out("Opuszczam swirla. Wpisz swirl(), aby powrócić.", skip_after=TRUE)
  invisible()
}
...
info <- function(){
  ...
  swirl_out("-- Wpisanie skip(), pozwala (czasami) pominąć aktualne pytanie.", skip_before=FALSE)
  swirl_out("-- Wpisanie play() pozwala powrócić do konsoli R i poeksperymentować na własną rękę; swirl zignoruje to co będziesz wtedy robił...", skip_before=FALSE)
  swirl_out("-- AŻ DO MOMENTU GDY wpiszesz nxt() co sprawi, że swirl znowu zacznie się interesować tym co robisz.", skip_before=FALSE)
  swirl_out("-- Wpisanie bye() powoduje wyjście ze swirla. Twój postęp zostanie zachowany.", skip_before=FALSE)
  swirl_out("-- Wpisanie main() przenosi do menu głównego swirla.", skip_before=FALSE)
  swirl_out("-- Wpisanie info() wyświetla opis, który właśnie kończysz czytać.", skip_before=FALSE, skip_after=TRUE)
  invisible()
}
...
resume.default <- function(e, ...){
  ...
  esc_flag <- TRUE
  on.exit(if(esc_flag)swirl_out("Opuszczam swirla. Wpisz swirl(), aby powrócić.", skip_after=TRUE))
  ...
  if(uses_func("skip")(e$expr)[[1]]){
    ...
    if(is(e$current.row, "script") && is.na(correctAns)) {
      ...
      if(file.exists(correct_script_path)) {
        ...
        swirl_out("Otworzyłem właśnie skrypt z możliwym rozwiązaniem.",
                  skip_after=TRUE)
        file.edit(correct_script_path)
        swirl_readline("Wciśnij Enter kiedy będziesz gotowy do kontynuowania...")
      }
    ...
    # If this is not a script question...
      ...
      swirl_out("Wprowadzam poprawną odpowiedź za Ciebie...",
                skip_after=TRUE)
  ...
  while(!e$prompt){
    ...
    if(e$row > min(nrow(e$les), e$test_to)) {
      ...
      swirl_out("Dotarłeś do końca lekcji! Wracam do menu głównego...")
