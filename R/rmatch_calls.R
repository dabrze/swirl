...
rmatch_calls <- function(expr, eval_for_class=FALSE, eval_env=NULL){
  ...
  tryCatch(fct <- match.fun(expr[[1]]),
           error=function(e)stop(paste0("Niepoprawne wyrażenie ", dprs(expr), 
                                        ": ", dprs(expr[[1]]), " nie jest funkcją.\n")))
  ...
  if(is(fct, "standardGeneric")){
    stop(paste0("Niepoprawne wyrażenie, ", dprs(expr), ": ", dprs(expr[[1]]), " jest typu standardGeneric.\n"))
  }
  ...
  if(isS3(fct)){
    ...
    if(!is.atomic(expr[[2]]) & !eval_for_class){
      stop(paste0("Niepoprawne wyrażenie, ", dprs(expr),": Pierwszy argument, ", dprs(expr[[2]]), 
                  ", do metody S3'", dprs(expr[[1]]), 
                  "', jest ", class(expr[[2]]) , ", więc nie jest atomowy,",
                  " zatem jego klasa nie może zostać określona za pomocą drzewa AST",
    ...
    if(is(err, "try-error")){
      tryCatch(fct <- getS3method(as.character(expr[[1]]), "default"),
               error = function(e)stop(paste0("Niepoprawne wyrażenie ", dprs(expr), ": ",
                                              "Brakuje pasującej metody S3 lub wartości domyślnej dla obiektu, ",
